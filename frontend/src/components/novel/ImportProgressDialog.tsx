...
export default function ImportProgressDialog({ open, progress, error, skippedCount, skippedChapters, onClose }: Props) {
      ...
      <div className="relative w-[420px] max-w-[90vw] rounded-xl border border-border bg-background p-6 shadow-2xl">
        <div className="flex items-start gap-3">
          <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${
            isError ? 'bg-danger-bg text-destructive' : isDone ? 'bg-tag-green text-tag-green-foreground' : 'bg-primary/10 text-primary'
          }`}>
            {isError ? (
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-sm text-muted-foreground">{STAGE_LABEL[progress.stage]}</p>
          </div>
          {chapterText && (
